...
`<input onChange={handleChange}></input>` のような onChange って使うべきなのかなぁって疑問に思っているので自分の考えをまとめます。
公式の例は onChange にハンドラを渡しているのですが、これが良いとは言い切れないケースもそれなりにあるよなぁと個人的に思っています。
自信があるわけでもないので間違っていたら「優しく」指摘してくれると助かります。
...
その結果、入力のたびにその内容を state に書き込むので、state を配置している階層によっては再レンダリングがたくさん走ります。

コンポーネントの分割や memo などを駆使すればある程度緩和ができるものだとは思いますが、これらはコンポーネントが更新されないといった事故の元にもなるので、既存コードへの memo の導入は慎重にならなければいけません。
特に誰かから引き継いだコードだと state 周りのリファクタリングはそれ用の工数をもらわないと厳しいところがあります。

ただ一応（近い？遠い？）将来的には Concurrent mode によって緩和されるはずなので気にしなくていい問題になるのかもしれません。

### 制御したい場合のユースケースは本当に意味があるものなのか
...
入力後、つまり onBlur のタイミングで行ってもいいのではと思っており、onBlur を使うと入力時に再レンダリングによってもっさりするといった問題は解消できます。
あと個人的には入力内容を onChange でライブ変換されるのは使いにくいと思っており、本当に入力を React 側からリアルタイムで制御する必要はあるのでしょうか。

唯一、他のフォームの入力状態に応じて入力内容を変換するというユースケースはありそうですが（たとえば二つの数値入力フォームがあって片方を入力するともう片方が合計 100 になるように自動で修正されるなど）、onBlur を使ったバリデーションや警告で勘弁してほしいと実装者都合では思っています。

**もっとも GUI エディタのように動的に制御できるフォームを作りやすくなったという恩恵を React がもたらしているとは思うので onChange 軸の制御されたコンポーネントを否定することはできないのですが、少なくとも動的に入力内容を制御しいなくていいユースケースなら onChange や 制御されたフォームを使う必要はないと思います。**
...
そうすることで React の外の世界で非制御コンポーネントの制御が可能になります。
こうすることで React の state で value を管理しなくても、他のフォームの入力状態に応じて入力内容を変換するというユースケースにも対応できます。（=制御したい場合のユースケースも満たせる）

### 中途半端な制御コンポーネントとして使う

非制御コンポーネントを使うと入力時の再レンダリングの問題から完全に開放されるのでこれに寄せたい気持ちが強いのですが、ref を使うのは少し抵抗があったりします。
ref 自体も ADVANCED GUIDES の内容なのでチーム構成によっては導入が難しかったり、たくさん項目があるフォームの場合それぞれに ref を持たせるのはめんどくさかったりします。
...
onBlur は万能な気もするのですが、input 一つのフォームでエンターキー押して submit のような実装だと onBlur は実行されないので、form の作りによっては使えません。
...
(気合入れないといけないときは onChange + debounce もしています。)
