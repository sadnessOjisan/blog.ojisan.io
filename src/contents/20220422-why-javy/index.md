...
その上で wasm の実行方法を考えてみると、当然コンピュータ上で直接実行できないので、wasm を各 PC で動くネイティブコードに変換するか、パソコンになんらかのランタイムを入れると良い。
...
でもそれだと最初から QuickJS を バイトコードにして配布すればいいし、QuickJS で JS を実行するのであれば手元で NodeJS を実行すればいいだけなので嬉しさはないと思うかもしれないが、ここで登場するのが wizer だ。

結果から言うと正直あまり理解できていない。（大事なことだから２回言った）
