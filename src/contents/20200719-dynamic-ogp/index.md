...
path: /dynamic-ogp
created: "2020-07-19"
title: タダで運用できる動的OGP生成のアーキテクチャを決めるまでの苦悩
visual: "./visual.png"
tags: [OGP]
---

OGP 生成をタダでやろうとすると意外と苦労した話です。
アプリケーションの技術スタックは React/NextJS、クラウドサービスとして CloudStorage, Cloud Function, Vercel での検証です。

動的 OGP それ自体のやり方は mizchi さんの記事やれとるときゃりーさんの実装を参照してみてください。

## (案 1: △) クライアントサイドで、OGP 画像を生成しその画像をオブジェクトストレージに保存しその 公開 URL を展開する

[ogpng](https://ogpng.vercel.app/)というサービスを作ったときのやり方です。
OGP 画像をクライアントサイドの canvas 上で生成して、それを画像化してクラウドストレージにアップロードします。
その後、ストレージの公開 URL の一部でクライアントサイドの詳細ページを作り、そのページの meta tag の`og:image`に公開 URL をセットすれば、その詳細ページの OGP 画像がセットされたこととなります。
...
このサービスがどうやって OGP を作っているかは[ブラウザ上で JSX をコンパイルしてプレビュー環境を作る](https://blog.ojisan.io/jsx-preview)にまとめていますので、気になる方は是非こちらもご覧ください。

## (案 2: ○) NodeJS 上で OGP 画像を生成し、オブジェクトストレージに保存しその 公開 URL を展開する

クライアント側で生成するのが嫌ならバックエンドで生成すればいいじゃないというのがこの案です。
ストレージには Cloud Storage を使うので、Cloud Function を利用します。
同じ Firebase プロダクトであれば、認証に使う環境変数が不要なためです。
...
実際には背景画像にタイトルとかを埋め込んだりしたいと思うのですが、今は API の話について書きたいので、画像の作り方については割愛します。
最近だと[mizchi さんがブログで OGP 対応をした話](https://mizchi.dev/202006211925-support-ogp)や、[れとるときゃりーさんがサービスの OGP 用の API のソースコードをまるごと公開してくれている](https://github.com/retoruto-carry/shindan-chart-maker-ogp)ので、そちらを参照すると良いでしょう。
([診断チャートメーカーのソースコード](https://github.com/retoruto-carry/shindan-chart-maker)が丸ごと公開されているのはとても参考になりました。)

画像を生成してしまえばあとはこれを Cloud Storage に上げてしまえば OK です。
...
NextJS を使っていると別のホスティングサービスから叩いているはずなので、別ドメインからのリクエストに対する対応、つまり CORS の許可が必要になります。
...
さらに飛んできたリクエストメソッドが preflight かどうかを Request Method が OPTIONS かどうかで判断し、OPTIONS であれば適切なヘッダを返しています。
...
とりあえず 2021 年までは大丈夫そうなのですが、なぜか v8 でデプロイすると表示されません。
...
どうやら無料で Firebase Function を使うことができなくなった + これからできなくなるので、この案 2 は技術的な要件には答えられるもののやめたくなりました。
...
パスを指定して`getDownloadURL` で公開 URL を作れる便利なメソッドはないですが、`getSignedUrl` という署名付き URL を作ることができます。
...
node-canvas の都合上、NodeJS をタダで運用する必要があるのですが、それは Heroku か Vercel しか知りません。（他にあったら教えてください）
Heroku はダウンタイムみたいなのがあったはずなので安定した運用は難しそうなので Vercel を選んでいます。
Vercel からオブジェクトの永続化はファイルへの書き込みの観点から難しそうなので、直接バイナリを返しています。
...
また、いくらお金を気にしなくていいと言ったって、環境への負荷を考えると案 6 はやりたくないです。
OGP を誰かが生成するたびに Function が実行されて電気を消費している気がしています。
プロは環境にも配慮すべきだと思います。（知らんけど）

### sadnessOjisan はどうしてるの？

ogpng の実装は 1, それ以外は 6 でやってます。
両方とも完全に無料で動的な OGP を作れます。
個人開発やちょっとした実験に対してはお金を払いたくないため、課金回避しています。
しかし、もし私がお金持ちならばきっと案 4 を選んでいることでしょう。
